...
class Raspicam_driver : public ThreadedDriver
{
  ...
  private:
    ...
    int configure(int FRAME_WIDTH, int FRAME_HEIGHT);
    int capturar();
    Mat Raspi_Capture();
...
int Raspicam_driver::configure(int FRAME_WIDTH, int FRAME_HEIGHT)
{
  //tCamera.open();
    ...
    : ThreadedDriver(cf, section, false, PLAYER_MSGQUEUE_DEFAULT_MAXLEN) //pode colocar a interface que o driver proverá
{
 ...
 /* if(configure(350,350)==-1)
  {
    this->SetError(-1);
    return;
  }*/
  height = cf->ReadInt(section, "height", 350);
  width = cf->ReadInt(section, "width", 350);
...
int Raspicam_driver::capturar()
{
  ...
  // IplImage frame_ipl;
  // Mat frame_mat;
 // unsigned char mat[height*width*channels*sizeof(unsigned char)];
//      int i;
  ...
  //usleep(delay);
     ...
     //imwrite("oijoice.png",frame_mat);
      // frame_ipl = frame_mat;  //conversão Mat para IplImage
      // frame = &frame_ipl;
      // imwrite("balbal.png",frame);
      ...
      frame.release();
      // memcpy(mat, frame.ptr(), data->image_count);
      // data->image = mat;
      switch (data->bpp)
      {
      case 8:
      ...
      case 24:
        ...
        // for (i = 0; i < static_cast<int>(data->image_count); i++)//; i += (frame->nChannels))
        // {
        //   data->image[i] = frame.at<uint8_t>(i);
        //   // data->image[i] = frame->imageData[i];
        //   // data->image[i + 1] = frame->imageData[i + 1];
        //   // data->image[i + 2] =  frame->imageData[i + 2];
     ...
     // imshow("DISPLAY", teste);
     // waitKey(10);
     // imwrite("teste.jpg", teste);
      assert(data->bpp > 0);
