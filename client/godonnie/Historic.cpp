...
string Historic::show() {
	...
	history << "Comando " << cmdNum << " foi " << (*iterator).name << ", " << (*iterator).feedback << ", posição [" << (*iterator).posx << ", " << (*iterator).posy << ", " << (*iterator).degree << "]" << endl;
	cmdNum++;
    }
...
string Historic::getLast() {
	...
	history << "Comando " << commandsList.size() - 1 << " foi " << last.name << ", " << last.feedback << ", posição [" << last.posx << ", " << last.posy << ", " << last.degree << "]";  // dont add newline here !
    }
