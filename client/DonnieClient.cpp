...
void DonnieClient::ParaFrente(float arg)
{
	...
	robot->Read();
	if(sonarProxy->GetRange(1) > 0.2) // For donnie, do not work in stage.
	{
	  stop = false;
	...
	while(!stop)
	{
	  ...
	  if(passos > Npassos )
	  {
	    ...
	    cout << "Npassos " << Npassos << endl;
	    //cout << "Parado apos " << Npassos << " passos" << endl;
	    break;
	  }

	  robot->ReadIfWaiting();
	  if(this->FrontBumper() != 0 or sonarProxy->GetRange(1) < 0.06 or sonarProxy->GetRange(2) < 0.08 or sonarProxy->GetRange(0) < 0.08 ) // For donnie, do not work in stage.
	  {
	  	//p2d_headProxy->SetSpeed(0,0);
	    p2dProxy->SetSpeed(0,0);
	    stop = true;
	    ...
	    cout << "Possivel caminho obstruido..." << endl;
	  	cout << "Bumper:" << this->BackBumper() << endl;
	  	cout << "Sonar 3:" << sonarProxy->GetRange(0) << endl;
	  	cout << "Sonar 4:" << sonarProxy->GetRange(1) << endl;
	  	cout << "Sonar 5:" << sonarProxy->GetRange(2) << endl;
	    break;
	  }
	    
	  robot->ReadIfWaiting();
	  if(sonarProxy->GetRange(1) < 0.2 and !obstacle) // For donnie, do not work in stage.
	  {
	    Npassos = passos;
	    obstacle = true;
	  }
	  ...
	  robot->ReadIfWaiting();
	  if(hypotf(p2dProxy->GetXPos() - posxi, p2dProxy->GetYPos() - posyi) > path[passos]) //marques - tirei o 0.15
	  {
	  		passos++;
	  }
...
void DonnieClient::ParaTras(float arg)
{
	...
	if(sonarProxy->GetRange(3) > 0.2) // For donnie, do not work in stage.
	{
	  stop = false;
	  //p2d_headProxy->SetSpeed(-1,0);
	  p2dProxy->SetSpeed(-1,0);
	}
	else cout << "Caminho obstruido sensor 3:" << sonarProxy->GetRange(3) << endl;
	...
	while(!stop)
	{
	  ...
	  robot->ReadIfWaiting();
	  if(this->BackBumper() != 0 or sonarProxy->GetRange(4) < 0.06 or sonarProxy->GetRange(3) < 0.08 or sonarProxy->GetRange(5) < 0.08) // For donnie, do not work in stage.
	  {
	  	//p2d_headProxy->SetSpeed(0,0);
	    p2dProxy->SetSpeed(0,0);
	    stop = true;
	    ...
	    cout << "Possivel caminho obstruido..." << endl;
	  	cout << "Bumper:" << this->BackBumper() << endl;
	  	cout << "Sonar 3:" << sonarProxy->GetRange(3) << endl;
	  	cout << "Sonar 4:" << sonarProxy->GetRange(4) << endl;
	  	cout << "Sonar 5:" << sonarProxy->GetRange(5) << endl;
	    break;
	  }
	  ...
	  if(sonarProxy->GetRange(4) < 0.2 and !obstacle) // For donnie, do not work in stage.
	  {
	    Npassos = passos;
	    obstacle = true;
	    cout << "Caminho obstruido sensor 4:" << sonarProxy->GetRange(4) << endl;
	  }
...
void DonnieClient::ParaDireita(float arg)
{
	...
	//if(yawi>M_PI) yawi = -M_PI+(yawi-M_PI); //marques
	//else if(yawi<-M_PI) yawi = M_PI-(yawi+M_PI); //marques

	cout << "Initial:" << yawf << endl; //marques
	//cout << "Initial:" << yawi << endl;
	...
	//if(yawd>M_PI) yawd = -M_PI+(yawd-M_PI); //marques
	//else if(yawd<-M_PI) yawd = M_PI-(yawd+M_PI); //marques

	cout << "Destiny:"<< yawd << endl;
	...
	p2dProxy->SetSpeed(0,0);
	robot->Read();
	cout << "Final:" << p2dProxy->GetYaw() << endl;
}
